...
function renderServers(servers) {
    ...
    servers.forEach(server => {
        ...
        html += `
            <div class="server-card" onclick="window.open('${server.url}', '_blank')" onkeydown="if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); window.open('${server.url}', '_blank'); }" tabindex="0" role="button" aria-label="${server.name} - ${server.category}">
                ...
                <div class="server-footer">
                    <a href="${server.url}" class="server-url" target="_blank" onclick="event.stopPropagation()">
